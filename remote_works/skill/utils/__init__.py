...
def get_skill_images(skill_type):
    """Return list of skill images that will be placed in skill gallery."""
    return list(skill_type.images.all())
...
def calculate_revenue_for_variant(variant, start_date):
    ...
    for task_line in variant.task_lines.all():
        if task_line.task.created >= start_date:
            net = task_line.unit_price_net * task_line.quantity
            gross = task_line.unit_price_gross * task_line.quantity
