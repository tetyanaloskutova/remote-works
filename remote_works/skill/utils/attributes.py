def get_skill_attributes_data(skill):
    """Returns attributes associated with the skill,
    as dict of Attribute: AttributeValue values.
    """
    attributes = skill.skill_type.skill_attributes.all()
    attributes_map = {
        attribute.pk: attribute.translated for attribute in attributes}
    values_map = get_attributes_display_map(skill, attributes)
    return {
...
def get_name_from_attributes(variant, attributes):
    """Generates SkillVariant's name based on its attributes."""
