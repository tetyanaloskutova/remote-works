...
def skill_list(request):
    skills = Skill.objects.prefetch_related('images')
...
def skill_details(request, pk):
    skills = Skill.objects.prefetch_related('variants', 'images').all()
    skill = get_object_or_404(skills, pk=pk)
    variants = skill.variants.all()
    images = skill.images.all()
...
def variant_images(request, skill_pk, variant_pk):
    skill = get_object_or_404(Skill, pk=skill_pk)
    qs = skill.variants.prefetch_related('images')
...
@permission_required('skill.manage_skills')
def skill_images(request, skill_pk):
    skills = Skill.objects.prefetch_related('images')
    skill = get_object_or_404(skills, pk=skill_pk)
    images = skill.images.all()
    ctx = {
        'skill': skill, 'images': images, 'is_empty': not images.exists()}
    return TemplateResponse(
        request, 'dashboard/skill/skill_image/list.html', ctx)


@permission_required('skill.manage_skills')
def skill_image_create(request, skill_pk):
    skill = get_object_or_404(Skill, pk=skill_pk)
    skill_image = SkillImage(skill=skill)
    ...
    ctx = {'form': form, 'skill': skill, 'skill_image': skill_image}
    return TemplateResponse(
        request,
...
@permission_required('skill.manage_skills')
def skill_image_edit(request, skill_pk, img_pk):
    skill = get_object_or_404(Skill, pk=skill_pk)
    skill_image = get_object_or_404(skill.images, pk=img_pk)
    form = forms.SkillImageForm(
        request.POST or None, request.FILES or None, instance=skill_image)
    if form.is_valid():
        skill_image = form.save()
...
@require_POST
def ajax_reorder_skill_images(request, skill_pk):
    skill = get_object_or_404(Skill, pk=skill_pk)
