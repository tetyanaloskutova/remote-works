...
from ...skill.models import Attribute


def filter_skills_by_attributes(qs, filter_value):
...
def filter_skills_by_price(qs, price_lte=None, price_gte=None):
    if price_lte:
...
def filter_skills_by_categories(qs, categories):
    categories = [
...
def filter_skills_by_collections(qs, collections):
    return qs.filter(collections__in=collections)


def sort_qs(qs, sort_by_skill_order):
    if sort_by_skill_order:
        qs = qs.order_by(sort_by_skill_order['direction']
                         + sort_by_skill_order['field'])
    return qs
