...
from ...skill.types import SkillType
...
class AttributeMixin:
    ...
    @classmethod
    def clean_attribute(
            cls, instance, cleaned_input, errors, skill_type=None):
        ...
        query = models.Attribute.objects.filter(slug=slug).filter(
            Q(skill_type=skill_type)
            | Q(skill_variant_type=skill_type))
        query = query.exclude(pk=getattr(instance, 'pk', None))
        if query.exists():
            cls.add_error(
                errors, 'name',
                'Attribute already exists within this skill type.')
        return cleaned_input
...
class AttributeCreate(AttributeMixin, ModelMutation):
    ...
    skill_type = graphene.Field(
        SkillType,
        description='A skill type to which an attribute was added.')

    class Arguments:
        id = graphene.ID(
            required=True,
            description='ID of the SkillType to create an attribute for.')
        type = AttributeTypeEnum(
            required=True,
            description=(
                'Type of an Attribute, if should be created for Skills '
                'or Variants of this SkillType.'))
    ...
    def mutate(cls, root, info, id, type, input):
        errors = []
        skill_type = cls.get_node_or_error(
            info, id, errors, 'id', SkillType)
        if not skill_type:
        ...
        cls.clean_attribute(
            instance, cleaned_input, errors, skill_type=skill_type)
        ...
        if type == AttributeTypeEnum.VARIANT.name:
            skill_type.variant_attributes.add(instance)
        else:
            skill_type.skill_attributes.add(instance)
        cls._save_m2m(info, instance, cleaned_input)
        return AttributeCreate(
            attribute=instance, skill_type=skill_type, errors=errors)
...
class AttributeUpdate(AttributeMixin, ModelMutation):
    ...
    skill_type = graphene.Field(
        SkillType, description='A related skill type.')
...
class AttributeDelete(ModelDeleteMutation):
    ...
    def success_response(cls, instance):
        ...
        response.skill_type = (
            instance.skill_type or instance.skill_variant_type)
        return response
