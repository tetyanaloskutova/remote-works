...
def test_view_ajax_reorder_skill_images(admin_client, skill_with_images):
    order_before = [img.pk for img in skill_with_images.images.all()]
    ...
    assert response.status_code == 200
    order_after = [img.pk for img in skill_with_images.images.all()]
    assert order_after == ordered_images


def test_view_ajax_reorder_skill_images_invalid(
